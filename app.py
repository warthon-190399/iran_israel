...
st.set_page_config(
    page_title="Iran vs Israel: Análisis de Sentimiento",
    page_icon=":newspaper:",
    layout="wide",
...
# Título principal
st.title("Conflicto Irano-Israelí")

# Texto introductorio
st.write("Bienvenido a un análisis de sentimiento sobre el conflicto Irano-Israelí. En esta aplicación, puedes explorar el análisis de sentimiento realizado sobre diferentes fuentes de noticias como Al Jazeera, CNN, New York Post y Russia Today.")
...
def analyze_sentiment_textblob(text):
    # Crear un objeto TextBlob con el texto dado
    blob = TextBlob(text)
    # Obtener la puntuación de sentimiento (polarity) del texto
...
selected_df["Categoria_Polaridad"] = selected_df["Sentimiento"].apply(nombre_polaridad)
...
fig_bar_polarG = px.bar(conteo_polaridad_diario,
                                   ...
                                   labels={'Categoria_Polaridad': 'Categoría de Polaridad',
...
fig_sentiment_vader = px.histogram(selected_df,
                                   ...
                                   labels={'Sentimiento': 'Polaridad del Sentimiento'}
                                   )
...
fig_bubble = px.scatter(selected_df,
                        ...
                        labels={'Fecha': 'Hora de Publicación',
                                'Sentimiento': 'Polaridad del Sentimiento'})
...
#%%
df_sentimiento_promedio = selected_df.groupby(['Dia', 'Diario'])['Sentimiento'].mean().reset_index()
...
fig_line = px.line(df_sentimiento_promedio,
                    ...
                    template="plotly_dark",
                    height=350,
                    labels={'Dia': 'Día', 'Sentimiento': 'Sentimiento Promedio'})  # Usar 'Dia' en lugar de 'dia
